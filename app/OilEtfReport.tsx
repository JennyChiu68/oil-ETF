...
function OilEtfFundReport({
  ...
  const premiumAbs = Math.abs(snapshot.current.premiumDiscountPct);
  const premiumState =
    premiumAbs < 0.005 ? "正常" : premiumAbs < 0.01 ? "关注" : "偏离";
      ...
      <div className="report-phone">
        ...
        <section className="card monitor-card">
          ...
          <div className="monitor-grid">
            <div>
              <span>市场价格</span>
              <strong>{formatUsd(snapshot.current.marketPrice)}</strong>
              <small>
                买 {formatUsd(snapshot.current.bid)} · 卖{" "}
                {formatUsd(snapshot.current.ask)}
              </small>
            </div>
            <div>
              <span>较NAV溢折价</span>
              <strong
                className={
                  snapshot.current.premiumDiscountPct >= 0 ? "up" : "down"
                }
              >
                {formatSigned(
                  snapshot.current.premiumDiscountPct * 100,
                  2,
                  "%",
                )}
              </strong>
              <small>监测阈值 ±0.50% / ±1.00%</small>
            </div>
          </div>
          <div className="premium-scale" aria-label="当前溢折价位置">
            <span className="scale-negative">折价</span>
            <span className="scale-center">NAV</span>
            <span className="scale-positive">溢价</span>
            <i
              style={{
                left: `${Math.max(
                  3,
                  Math.min(
                    97,
                    50 + snapshot.current.premiumDiscountPct * 2500,
                  ),
                )}%`,
              }}
            />
          ...
          <p className="inline-note">
            该指标基于USCF最新NAV和官方市场价格快照，仅反映当日收盘口径。
          </p>
        </section>
        ...
        <section className="card monitor-card">
          <div className="section-heading">
            <div>
              <p className="section-kicker">EXTREMES &amp; ROLL</p>
              <h2>极值与展期监控</h2>
            </div>
            <span className="date-badge">当前区间</span>
          </div>
          <div className="monitor-grid">
            <div>
              <span>最大资产增加</span>
              <strong className="up">
                {analysisStats.biggestUp
                  ? formatSigned(analysisStats.biggestUp.value, 2)
                  : "--"}
              </strong>
              <small>
                {analysisStats.biggestUp?.row.date ?? "--"} · {analysisSuffix}
              </small>
            </div>
            <div>
              <span>最大资产减少</span>
              <strong className="down">
                {analysisStats.biggestDown
                  ? formatSigned(analysisStats.biggestDown.value, 2)
                  : "--"}
              </strong>
              <small>
                {analysisStats.biggestDown?.row.date ?? "--"} ·{" "}
                {analysisSuffix}
              </small>
            </div>
          </div>
          <div className="roll-note">
            <div className="roll-icon" aria-hidden="true">
              <span />
            </div>
            <div>
        ...
        <section className="card methodology-card">
          <div className="section-heading">
            ...
            <span className="verified-badge">已核验</span>
          </div>
          <div className="method-list">
            ...
            <div>
              <span>02</span>
              <p>{snapshot.methodology.netAssets}</p>
            </div>
            <div>
              <span>03</span>
